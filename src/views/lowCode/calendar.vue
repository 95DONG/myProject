<template>
  <div class="zhiban-box">
    ...
    <!-- 测试 -->
    <div>
      <div v-for="(month, monthIndex) in testArr" :key="monthIndex">
        <div class="week">
      ...
      <!-- 正常 -->
      <div class="week" v-if="true">
        <span v-for="(week, index) in weekday" :key="index">{{ week }}</span>
      </div>
      <div class="day-box" v-if="true">
...
export default {
  ...
  methods: {
    ...
    initCalendar (year, month) {
      ...
      // new Date(year + 1, 0, 1).getTime() - 8640000 新年第一天的前一天
      ...
      if (calendar[calendar.length - 1].length < 7) {
        const arr = 7 - calendar[calendar.length - 1].length
    ...
    createDay (currentYear, currentMonth) {
      const curDate = new Date(currentYear, currentMonth, 0)
      for (let k = 1; k <= curDate.getDate(); k++) {
    ...
    changeColorWeek (currentYear, currentMonth) {
      ...
      // 改变周六日颜色
      for (let i = 0; i < this.dayArry.length; i++) {
        if (i % 7 == 0 && i / 7 != 0) {
