<template>
  <div>
    ...
    <el-input
      v-model="key"
      placeholder="请输入歌曲/歌手名11"
      style="width: 40%"
      @keyup.enter.native="getmusic"
      clearable
    ></el-input>
    ...
    <!--歌曲列表-->
    <p>
      共<span style="color: #ffd04b">{{ tableData.total }}</span
      >条数据
    </p>
    <el-table
      :data="tableData.list"
    ...
    >
      <el-table-column prop="pic" label="图片" width="80px">
        <template slot-scope="scope">
          <el-image
            v-if="scope.row.pic !== '' && scope.row.pic !== null"
            :src="scope.row.pic"
    ...
    <!--自定义样式【电脑端】-->
    <div class="audioBox" v-show="JSON.stringify(nowmusic) !== '{}'">
      <!--歌曲信息-->
    ...
    <!--自定义样式【手机端】-->
    <div
      class="audioBox_mobile"
      v-show="(JSON.stringify(nowmusic) !== '{}') & (isMobile === true)"
    >
      <!--时间线-->
...
export default {
  ...
  methods: {
    ...
    switchmusic (key) {
      ...
      // 上一首
      if (key === 'up') {
        const newid = this.nowmusic.id - 1
        // 判断是否是第一首，如果不是，正常播放上一首，如果是，循环播放倒数第一首
        if (newid > -1) {
          const now = this.musiclist[newid]
          now.id = newid
          // 获取音乐链接
          this.$api.getmusiclink(now.rid, type)
            .then(res => {
          ...
          const now = this.musiclist[this.musiclist.length - 1]
